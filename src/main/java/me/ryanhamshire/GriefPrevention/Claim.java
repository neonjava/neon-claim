...
public class Claim {
    ...
    public boolean isPrivacyEnabled = false;
    ...
    public boolean hasExplicitPermission(@NotNull Player player, @NotNull ClaimPermission level) {
        ...
        if (level == ClaimPermission.Manage) {
            for (String node : this.managers) {
                ...
                if (player.hasPermission(node.substring(1, node.length() - 1)))
                    return true;
            }
            return false;
        }
    ...
    private @Nullable Supplier<String> getDefaultDenial(
        ...
        if (player != null) {
            if (this.hasExplicitPermission(player, permission))
                return null;
        } else {
            if (this.hasExplicitPermission(uuid, permission))
                return null;
        }
     ...
     * {@link ClaimPermission}. Owner-exclusive edit actions
     * require {@code griefprevention.deleteclaims}. All other actions require
    ...
    /**
     * @deprecated Check {@link ClaimPermission#Container} with
     *             {@link #checkPermission(Player, ClaimPermission, Event)}.
     * @param player the Player
     * @return the denial message, or null if the action is allowed
     */
