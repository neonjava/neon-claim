...
public class ClaimCommand extends CommandHandler {
    ...
    public boolean onCommand(
        ...
        // /claim or /claimgui with no args -> GUI
        if (args.length == 0) {
            me.ryanhamshire.GriefPrevention.Claim claim = plugin.dataStore.getClaimAt(player.getLocation(), true,
                    playerData.lastClaim);
            if (claim == null) {
                GriefPrevention.sendMessage(player, TextMode.Err,
                        "§cYou are not in a claim. Go to where you have a claimed area.");
                return true;
            }

            // Check if owner, trusted admin (Manage permission), or global admin
            boolean isOwner = player.getUniqueId().equals(claim.ownerID);
            boolean isAdmin = claim.checkPermission(player, me.ryanhamshire.GriefPrevention.ClaimPermission.Manage,
                    null) == null;
            boolean isGlobalAdmin = player.hasPermission("griefprevention.adminclaims");

            if (isOwner || isAdmin || isGlobalAdmin) {
                me.ryanhamshire.GriefPrevention.ClaimGui.openMainGui(player, claim);
            } else {
                GriefPrevention.sendMessage(player, TextMode.Err, "§cYou do not own this land and cannot manage it.");
            }
            return true;
        }
